...
def guardar_registro_google_sheets(fecha, comida, glicemia, carbohidratos, dosis):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            st.error(
                f"Error al guardar en la hoja (Código HTTP {response.status_code})"
            )
            return False
...
ALIMENTOS_CASEROS = [
    ...
    {
        "alimento": "Pan Hallulla / Marraqueta",
        "porcion": "1/2 unidad (50g)",
        "carbos": 25,
    },
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for p in datos.get("products", []):
                ...
                carbos_100g = float(
                    nutriments.get("carbohydrates_100g", 0.0) or 0.0
                )
                carbos_porcion = nutriments.get("carbohydrates_serving")
                carbos_porcion = (
                    float(carbos_porcion)
                    if carbos_porcion is not None
                    else None
                )
...
def buscar_usda(query):
    if not USDA_API_KEY:
        st.error(
            "⚠️ Falta configurar la API Key de USDA en los Secrets de Streamlit."
        )
        return []
        ...
        if response.status_code == 200:
            ...
            for item in datos.get("foods", []):
                ...
                for nut in item.get("foodNutrients", []):
                    if nut.get("nutrientId") == 1005 or "Carbohydrate" in nut.get(
                        "nutrientName", ""
                    ):
                ...
                if serving_size_val:
                    ...
                    carbos_porcion = round(
                        (carbos_100g * serving_size_val) / 100.0, 1
                    )
                else:
...
with pestana1:
    ...
    comida_actual = next(
        ...
        if c["nombre"] == comida_sel_nombre
    )

    st.caption(f"Ratio activo: **1U por cada {comida_actual['ratio']}g CH**")

    carbos_desde_plato = sum(
        [item["total_carbos"] for item in st.session_state.plato]
    )
    ...
    if st.session_state.plato:
        st.info(
            f"🛒 Carbohidratos del plato activo: **{round(carbos_desde_plato, 1)}g**"
        )

    paso_dosis = st.radio(
        "Redondeo de dosis (pasos de):", [0.5, 1.0], horizontal=True
    )

    if st.button(
        "Calcular Dosis Total y Guardar", type="primary", use_container_width=True
    ):
        ...
        dosis_comida = (
            carbos / comida_actual["ratio"]
            if comida_actual["ratio"] > 0
            else 0.0
        )
        ...
        st.markdown("---")
        st.metric(
            label="Dosis Sugerida Final", value=f"{total_redondeado:.1f} U"
        )

        st.write("**Detalle del Cálculo:**")
        if rango_encontrado:
            st.write(
                f"- Corrección por glicemia (Rango `{rango_encontrado}`): **+{dosis_correccion} U**"
            )
        else:
            st.write(
                "- Corrección por glicemia: **+0.0 U** (Glicemia fuera de rangos)"
            )

        st.write(
            f"- Bolus por comida ({carbos}g / {comida_actual['ratio']}): **+{dosis_comida:.2f} U**"
        )
        ...
        if exito:
            ...
            st.session_state.historial_local.append(
                {
                    "Fecha": fecha_str,
...
with pestana2:
    ...
    if origen == "Tabla Rápida Casera":
        ...
        sel = st.selectbox(
            "Selecciona alimento:", df_caseros["alimento"].tolist()
        )
        item = df_caseros[df_caseros["alimento"] == sel].iloc[0]

        st.caption(
            f"Porción estándar: **{item['porcion']}** = **{item['carbos']}g CH**"
        )
        ...
        if busqueda:
            ...
            if resultados:
                ...
                info_prod = next(
                    r for r in resultados if r["nombre"] == prod_sel
                )

                st.markdown("---")
                st.write(
                    f"**Porción referencial:** `{info_prod['serving_size']}`"
                )

                col_a, col_b = st.columns(2)
                with col_a:
                    st.metric(
                        "CH por 100g / 100ml", f"{info_prod['carbos_100g']}g"
                    )
                with col_b:
                    ch_p = (
                ...
                if modo_calculo == "Gramos / ml exactos":
                    ...
                    total_ch = round(
                        (info_prod["carbos_100g"] * gramos) / 100.0, 1
                    )
                    ...
                    if info_prod["carbos_porcion"] is not None:
                        ...
                        total_ch = round(
                            info_prod["carbos_porcion"] * n_porciones, 1
                        )
                        detalle_txt = f"{n_porciones} porción(es) ({info_prod['serving_size']})"
                    else:
                        st.warning(
                            "Sin datos por porción. Utiliza gramos exactos."
                        )
                        total_ch = 0
                        detalle_txt = ""

                if total_ch > 0 and st.button(
                    "Añadir al Plato", use_container_width=True
                ):
                    st.session_state.plato.append(
                        {
                    ...
                    st.success(
                        f"Añadido {info_prod['nombre']} ({total_ch}g CH)"
                    )
            else:
    ...
    if st.session_state.plato:
        ...
        st.write(
            f"**Total Carbohidratos acumulados:** {round(sum(df_plato['total_carbos']), 1)}g"
        )
...
with pestana3:
    ...
    if st.session_state.historial_local:
        ...
        st.info("Los registros que vayas guardando aparecerán aquí durante la sesión.")
    ...
    for ci, c in enumerate(st.session_state.config["comidas"]):
        with st.expander(f"⚙️ Configurar Ratios y Rangos para {c['nombre']}"):
            ...
            for ei, e in enumerate(escalas):
                ...
                with col_min:
                    e["min"] = st.number_input(
                        "Min", value=int(e["min"]), key=f"min_{ci}_{ei}"
                    )
                with col_max:
                    e["max"] = st.number_input(
                        "Max", value=int(e["max"]), key=f"max_{ci}_{ei}"
                    )
                with col_dos:
                ...
                with col_del:
                    if st.button("✕", key=f"del_{ci}_{ei}"):
                        ...
                        st.rerun()
            if st.button(
                f"+ Añadir Rango a {c['nombre']}", key=f"add_rango_{ci}"
            ):
